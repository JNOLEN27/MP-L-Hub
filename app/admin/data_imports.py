...
class DataImportsWindow(QMainWindow):
    ...
    def setupui(self):
        ...
        subtitle.setStyleSheet("color: white; background-color: transparent; margin-bottom: 20px;")
    ...
    def createimportbutton(self, buttoninfo):
        ...
        if allowed:
            ...
            widget.setStyleSheet("""QWidget {background-color: #cccccc; border: none; border-radius: 8px; padding: 4px;}""")
        ...
        if not allowed:
            titlelabel.setStyleSheet("color: #666666; background-color: transparent;")
        ...
        if allowed:
            importbtn.setStyleSheet("""QPushButton {background-color: #cccccc; color: grey; border: none; padding: 8px; border-radius: 4px; font-weight: bold;} QPushButton:hover {color: green; background-color: #45a049;}""")
    ...
    def handleimport(self, buttoninfo):
        ...
        try:
            ...
            if not isvalid:
                QMessageBox.warning(self, "Validation Failed", "File validation failed:\n\n" + "\n".join(errors))
                return
            
            if errors:
                reply = QMessageBox.question(self, "Import with Warnings?", "File has warnings but can be imported:\n\n" + "\n".join(errors) + f"\n\nPreview shows {len(previewdata)} rows.\n\nProceed with import?", QMessageBox.Yes | QMessageBox.No)
